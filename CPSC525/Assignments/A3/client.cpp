...
void decrypt_block(unsigned char ctbuf[], size_t ctlen, int ifd[2], int ofd[2]) {
    unsigned char ptbuf[16];
    unsigned int xorbuf[16] = { 0 };
    ...
    for (b = 15; b >= 0; b--) {
        ...
        for (i = 1; i <= 255; i++) {
            ...
            if (c == 'M') {
                printf("Byte: 0x%02x\n", i);
                ptbuf[b] = (unsigned char)(i ^ (16 - b));
                printf("PT Byte: 0x%02x\n", ptbuf[b]);
                printf("\n");
                xorbuf[b] = i;
                break;
...
int main(int argc, char * argv[]) {
        ...
        bool done = false;
        while (!done) {
            decrypt_block(ctbuf, ctlen, ifd, ofd);
            done = true;
        }
