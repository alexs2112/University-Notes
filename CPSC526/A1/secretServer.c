...
void readLineFromFd( int fd, char * buff, unsigned int len) {
    ...
    while(1) {
        ...
        // don't buffer overflow, leave one extra char for null terminator
        if (c++ >= len - 2) break;
    }
...
int writeStrToFd( int fd, const char * str) {
...
int main( int argc, char ** argv)
{
    ...
    while(1) {
        ...
        // Uncomment to see what the stored buffer and password looks like at this time
        // printf("Buffer: %s\nPassword: %s\n", globals.buffer, globals.password);
